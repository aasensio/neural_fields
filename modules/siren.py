...
# mapping network
class ConditioningNet(nn.Module):
...
if (__name__ == '__main__'):
    ...
    tmp = SirenNet(dim_in=dim_in, dim_hidden=dim_hidden, dim_out=1, num_layers=num_layers, w0_initial=30)
    
    x = np.linspace(-1, 1, 128)
    
    xin = torch.tensor(x[:, None].astype('float32'))


    out = tmp(xin).squeeze().detach().numpy()
